...
public class CategoryDetailView extends StandardDetailView<Category> {
    @ViewComponent
    private JmixSelect typeField;

    @ViewComponent
    private TypedTextField limitField;

    @Subscribe("typeField")
    public void onTypeFieldComponentValueChange(final AbstractField.ComponentValueChangeEvent<JmixSelect<OperationType>, OperationType> event) {
        if (event.getValue().equals(OperationType.ПРИХОД)){
            limitField.setEnabled(false);
        } else {
            Assert.isTrue(event.getValue().equals(OperationType.РАСХОД),
                    "Должен быть 'Расход'");
