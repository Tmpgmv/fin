...
public class TransferDetailView extends StandardDetailView<Transfer> {
  ...
  @ViewComponent private EntityPicker fromField;

  @ViewComponent private TextField amountField;

  @ViewComponent private TextField toField;
  ...
  private void createOperation(OperationType type) {
    ...
    operation.setDescription(String.format("%s %s", text, getEditedEntity().getTo()));
  ...
  private Category getCategory(OperationType type) {
    Category category =
        dataManager
            .load(Category.class)
            .query("select c from Category c where c.name = :name and c.type = :type")
