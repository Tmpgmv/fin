...
class LoginViewUiTest {
    ...
    void test_categoryIncomeViewAvailable() {
        ...
        viewNavigators.detailView(UiTestUtils.getCurrentView(), Category.class)
                .editEntity(category)
    ...
    void test_transferAvailable() {
        // 1. Load or create the Category entity for your test (use existing or create as needed)
        Transfer transfer = dataManager.load(Transfer.class)
                .id(UUID.fromString("019a6a2a-e94d-72da-b1a6-001e22935374"))
                .optional()
                .orElseThrow(() -> new IllegalStateException("Test Transfer not found"));

        // 2. Navigate to the DetailView for that entity
        viewNavigators.detailView(UiTestUtils.getCurrentView(), Transfer.class)
                .editEntity(transfer)
